...
JSECaptcha = (function() {
  ...
  #JSE-captcha-text { position: absolute; top: 17px; left: 50px; }\
  ...
  #JSE-captcha-footer { font-size: 7px; font-weight: bold; color: #888; position: absolute; bottom: 1px; left: 50px; }\
  ...
  this.submitMLData = function(passCallback,failCallback) {
    ...
    const url = 'https://load.jsecoin.com/captcha/request/';
    ...
    const data = JSON.stringify({"mlData": this.mlData});
    xhr.send(data);
  };
  ...
  this.loadGame = function(gameFile,cb) {
    ...
    var url = 'https://load.jsecoin.com/captcha/load/'+gameFile;
    xhr.open('GET', url);
    xhr.onload = function() {
      if (xhr.status === 200) {
